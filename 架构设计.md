...
平台业务能力与容器骨架严格分离。`app/src/` 维护基于 OpenSumi/CodeBlitz 的框架容器与文件系统；业务功能交互维护于 `taichu/app/extensions/{name}/`，按 VSIX 兼容扩展标准开发，打包编译成 `{name}-{version}.vsix`，传到 `registry/` 上架，`app/src` 运行时通过网络加载。

...
- 用户身份由环境变量注入（`USER_ID`、`TENANT_ID` 等，由 taixu 在 Pod 创建时设置），复用统一鉴权链路。
- MCP 服务端配置由 `opencode.json` 的 `mcp` 字段声明，**不**硬编码服务地址。

## 鉴权链路

- **不**在 dongfu 内维护登录态，**不**硬编码 API Key。
- MCP 服务调用的用户身份 Header 由 opencode 根据环境变量自动注入。
- 上游 taixu 网关按租户对 MCP 调用做限流、熔断与审计。

## 不在 dongfu 责任范围

- MCP 服务的发现与注册：由 LangHuan（[`../../langhuan/`](../../langhuan/)）维护；dongfu 仅消费。
- MCP 服务端实现：由业务方提供。
...
[`../设计文档.md`](../设计文档.md) 第四章「dongfu（洞府）」> 核心功能 4 与后端运行规范。
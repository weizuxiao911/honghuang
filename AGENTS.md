...
完整产品蓝图分两份文档存放，都是事实源：
- [`./功能设计.md`](./功能设计.md)：产品视角（定位/功能/价值/业务闭环）
- [`./架构设计.md`](./架构设计.md)：技术视角（分层/模块/VSIX生命周期/接口/数据/部署）

## 铁律

1. **设计文档为唯一事实源**。`功能设计.md` + `架构设计.md` 两份构成完整蓝图（产品/技术双视角）；任何与它们不一致的代码、配置、注释均视为错误。
...
- `功能设计.md`：产品蓝图（唯一事实源 · 产品视角）。
- `架构设计.md`：技术蓝图（唯一事实源 · 技术视角）。
...
- **必须用 `question` 确认**：
  - 改 `功能设计.md` / `架构设计.md` 的任何产品蓝图字段（状态机、协议、拓扑）
...
- **任何"是否争议 / 可能争议"的决策, 一律用 `question` 反馈, 不沿用默认**：
  - 上述"直接做"列表以外的所有改动, AI 默认按"可能争议"处理
  - 同一会话中即使之前用户已授权过类似动作, 下一次仍需重新决策, 不沿用"已通过"默认
  - 决策类动作包括但不限于: 文件位置选择、命名约定、删 / 合并 / 重构、API 设计、配置默认值、错误处理策略、依赖升级范围
  - 用户主动说"由你决定"/"你来"/"按你的意思"时, 才能自主做该决定, 并在 commit message 里说明授权来源
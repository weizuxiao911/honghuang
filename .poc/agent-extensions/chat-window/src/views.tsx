...
const ACTIVE_SESSION_KEY = 'agentnest.activeSessionId';
...
const STYLE_ID = 'agentnest-chat-window-style';
const CSS = `
...
const ChatWindow = () => {
  ...
  useEffect(() => {
    ...
    window.addEventListener('agentnest:session-changed', onChange as any);
    window.addEventListener('storage', onStorage);
    return () => {
      window.removeEventListener('agentnest:session-changed', onChange as any);
  ...
  return React.createElement(
    ...
    React.createElement(
      ...
      !sessionID
        ? React.createElement('div', { className: 'an-cw__empty' }, '请在左侧「AgentNest 会话」选择或新建对话')
        : rows.length === 0
...
exports['agentnest.chatWindow'] = ChatWindow;

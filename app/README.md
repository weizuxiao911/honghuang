...
详细职责、边界、与其它模块接口见 [`../设计文档.md`](../设计文档.md) 第一章「zifu（紫府）」。

## 当前状态

**已迁移 .poc/opensumi-web 完整代码**。本目录是 zifu 模块的正式工程位，包含：
...
├── README.md
├── AGENTS.md
├── package.json                # honghuang-zifu（CodeBlitz 容器构建）
├── tsconfig.json
├── webpack.config.js
├── .gitignore
...
│   ├── chat-window/            # 右侧对话窗口
│   │   ├── package.json        # name: zifu-chat-window
│   │   ├── tsconfig.json
│   │   ├── scripts/build.js
│   │   └── src/{extension.ts, views.tsx}
│   └── session-manager/        # 左侧会话管理
│       ├── package.json        # name: zifu-session-manager
│       ├── tsconfig.json
│       ├── scripts/build.js
...
前置：opencode 后端已起（[dongfu](../dongfu/) 或本地 opencode-serve，默认 `http://127.0.0.1:4096`）。
...
扩展清单默认从 `https://localhost:9000`（[langhuan](../langhuan/)）拉取。
...
- 不直连 dongfu（生产环境）；本地验证可直连。
- 不硬编码服务凭据；鉴权 Header 由 taixu 网关注入。
- 插件之间**仅**通过全局 command 命令 ID 联动。

## UI 设计（待迭代）

当前 UI 沿用 .poc 阶段的设计：左侧 `session-manager`（搜索 + 新对话 + 时间分组列表），右侧 `chat-window`（消息气泡 + 流式光标 + SSE 实时更新）。

UI 风格、交互细节、信息密度**待后续迭代**。本模块封装阶段先把骨架与端到端跑通，UI 由后续专门的设计迭代推进。

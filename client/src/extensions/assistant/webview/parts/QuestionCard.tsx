...
export const QuestionCard: React.FC<{
  ...
  return (
    <div className={`tc-q${answered ? ' is-done' : ''}`}>
      <div className="tc-q__head" onClick={() => setCollapsed((p) => ({ ...p, 0: !p[0] }))}>
        <span className="tc-q__badge">?</span>
        <span className="tc-q__head-title">
          问题 {questions.length} 已回答
        </span>
        <span className="tc-q__caret">{collapsed[0] ? '▸' : '▾'}</span>
      </div>
      {answered && (
        <div className="tc-q__summary">{summary(questions[0], 0)}</div>
      )}
      {!collapsed[0] && questions.map((q, qi) => (
        <div key={qi} className="tc-q__item">
          {q.header && <div className="tc-q__header">{q.header}</div>}
          <div className="tc-q__q">{q.question}</div>
            ...
            {q.options.map((opt, oi) => {
              ...
              return (
                <button
                  key={oi}
                  type="button"
                  className={`tc-q__opt${active ? ' is-active' : ''}`}
                  onClick={() => toggle(qi, opt.label, !!q.multiple)}
                  disabled={answered || submitting}
                >
                  <span className="tc-q__opt-mark">{q.multiple ? (active ? '☑' : '☐') : (active ? '◉' : '○')}</span>
                  <span className="tc-q__opt-body">
                    <span className="tc-q__opt-label">{opt.label}</span>
                    {opt.description && <span className="tc-q__opt-desc">{opt.description}</span>}
                  </span>
                </button>
              );
            })}
            {q.custom !== false && (
              <div className={`tc-q__opt tc-q__custom-opt${isCustomOn(qi) ? ' is-active' : ''}`}>
                <span className="tc-q__opt-mark">{q.multiple ? (isCustomOn(qi) ? '☑' : '☐') : (isCustomOn(qi) ? '◉' : '○')}</span>
                <span className="tc-q__opt-body">
                  <span className="tc-q__opt-label" onClick={() => { if (!isCustomOn(qi)) { setCustomActive(p => ({ ...p, [qi]: true })); } }}>
                    输入自己的答案
                  </span>
                  <textarea
                    className="tc-q__custom"
                    rows={1}
                    placeholder="输入你的答案..."
                    value={custom[qi] || ''}
      ...
      {!answered && (
        <div className="tc-q__foot">
          <button className="tc-q__submit" onClick={submit} disabled={submitting}>
            {submitting ? '提交中...' : '提交'}
          </button>
        </div>
      )}

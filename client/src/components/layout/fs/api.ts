 ...
 *
 * sandbox uri (sandbox:/workspace/foo) <-> 沙箱内 path (/workspace/foo) 的转换
 *   由 sandbox-fs.ts 负责, 本文件只接收/返回绝对 path 字符串.
 */
...
/**
 * 统一 fetch 封装 — 沙箱 HTTP API 调用
 *
 * 调用示例:
 *   fsFetch('GET', '/file', { params: { path: '/workspace/foo.txt' } })
 *   fsFetch('POST', '/pty', { body: { script: 'echo hi' } })
 */
